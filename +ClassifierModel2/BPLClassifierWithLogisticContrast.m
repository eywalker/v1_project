classdef BPLClassifierWithLogisticContrast < ClassifierModel.Classifier
    ...
    properties
        ...
        sigmaA = 1; % standard devation of category 'A'
        sigmaB = 10; % standard deviation of category 'B'
        ...
        alpha = 0; % parameters for logistic fit to sigma = exp(beta0 + beta1 * contrast)/(1 + exp(beta0 + beta1 * contrast)
        beta = -1;
    end
    
    methods (Static)
        function [muLL, listLL] = modelLogLikelihood(sigmaA, sigmaB, priorA, beta0, beta1, lapseRate, stimCenter, stimulus, contrast, response)
    ...
    methods
        function obj = BPLClassifierWithLogisticContrast(sigmaA, sigmaB, stimCenter)
        ...
        function [muLL, listLL] = getLogLikelihood(self, stimulus, contrast, response)
            ...
            [muLL, listLL] = ClassifierModel.BPLClassifier.modelLogLikelihood(self.sigmaA, self.sigmaB, self.priorA,...
                self.beta0, self.beta1, self.lapseRate, self.stimCenter, stimulus, contrast, response);
        ...
        function setModelParameters(self, paramValues)
            ...
            self.beta0 = paramValues(2);
            self.beat1 = paramValues(3);
            self.lapseRate = paramValues(4);
        end
        ...
        function paramSet = getModelParameters(self)
            ...
            paramSet.initValues = [self.priorA, self.beta0, self.beta1, self.lapseRate];
