classdef FixedLikelihoodMLPDecoder < handle
    ...
    methods
        function obj = FixedLikelihoodMLPDecoder()
...
%         end
        ...
        function L = getLikelihoodDistr(self, decodeOri, contrast, spikeCounts)
            ...
            shift = bsxfun(@plus, self.wo * x, self.bo);
            % note that shift has to be flipped because in the
            % implemenation in PyTorch, shift is the "shift in the grid
            % readout locations" - which is effectively sign flip of shift
            % in the likelihood function
            logL = shiftFunction(linspace(-1, 1, length(self.likelihood)), self.likelihood(:), -shift, 'linear', true);
        ...
        function configSet = getModelConfigs(self)
            ...
            configSet.likelihood = self.likelihood;
        ...
        function setModelConfigs(self, configSet)
            ...
            self.likelihood = configSet.likelihood;
