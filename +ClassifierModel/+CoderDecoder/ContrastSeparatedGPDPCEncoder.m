classdef ContrastSeparatedGPDPCEncoder < handle
    ...
    properties
        ...
        contrastSet; % set of contrasts on which the model was trained
    end
    
    methods
        function obj=ContrastSeparatedGPDPCEncoder(NUM_UNITS)
            if nargin < 1
        ...
        function train(self, stimulus, contrast, spikeCounts)
            ...
            contrastSet = sort(unique(contrast));
            ...
            self.trainStimulus = {};
            self.alpha = {}
            self.y_bias = zeros(length(contrastSet), self.NUM_UNITS);
            
            for idxCont = 1:length(contrastSet)
                contVal = contrastSet(idxCont);
                ...
                for indUnit = 1:self.NUM_UNITS
                    ...
                    y_bias(indUnit) = mean(y);

                    L = chol(K+sigma_obs^2*eye(size(K,1)),'lower');
                    alpha = L'\(L\(y'-y_bias(indUnit)));
                    alpha_c(indUnit,:) = alpha;
                end
                
                self.trainStimulus = [self.trainStimulus, {x}];
                self.alpha = [self.alpha {alpha_c}];
                self.y_bias(idxCont, :) = y_bias;
            end
            self.contrastSet = contrastSet;
        end
