classdef BehavioralBPLClassifierWithScaledWidth < handle
    ...
    properties
        ...
        params = {'priorA', 'lapseRate', 'a', 'beta', 'gamma', 'scale'};
        fixedParams = false(1, 6);
        p_lb = [0.25, 0, 0, 0, 0, 0]; % lower bound for parameters
        p_ub = [0.75, 0.5, 50, 8, 30, 10]; % upper bound for parameters
    ...
    methods
        ...
        function obj = BehavioralBPLClassifierWithScaledWidth(sigmaA, sigmaB, stimCenter, modelName, pwExtractor)
            ...
            if nargin < 4
                modelName = 'BPLClassifierWithScaledWidth';
            end
            if nargin < 1
                sigmaA = 3;
                sigmaB = 15;
        ...
        function pA = pRespA(self, dataStruct)
            ...
            if isfield(dataStruct, 'decodeOri') && isfield(dataStruct, 'likelihood')
                ...
                [~, sigma_dev] = self.pwExtractor(decodeOri, likelihood);% extract center and width of the likelihood function
            elseif isfield(dataStruct, 'likelihood_width')
                sigma_dev = dataStruct.likelihood_width;
            end
            ...
            sigma_offset = self.mapContrast(contrast);
            sigma = sigma_offset + self.scale * sigma_dev(:);
