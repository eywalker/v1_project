...
% 
    properties
        sigmaA = 3; % standard deviation of class 'A'
        sigmaB = 15; % standard deviation of class 'B'
        stimCenter = 270; % center of class distributions
    ...
    methods
        function obj = PSLLC(sigmaA, sigmaB, stimCenter, modelName, pointDecoderModel)
        ...
        % of the two stimulus distributions
            ...
            obj.pointDecoderModel = pointDecoderModel;
            obj.initializer = [];
            obj.initializer.stimCenter = @(n) randn(1, n) * 5 + 270;
        end
        
        function pointDecoder = getPointDecoder(self, dataSet)
            if ~isempty(self.pointDecoder)
                pointDecoder = self.pointDecoder;
                return;
            end
            
            if ~isempty(self.pointDecoderKey)
                key = self.pointDecoderKey;
            else
                key = dataSet.key;
                key.ptdec_id = self.pointDecoderModel;
                self.pointDecoderKey = key;
            end
            
            pointDecoder = getPointDecoder(cd_point.TrainedPointDecoder & key);
            
            % save it
            self.pointDecoder = pointDecoder;
        end
        ...
        function configSet = getModelConfigs(self)
            ...
            configSet.pointDecoderModel = self.pointDecoder;
            configSet.pointDecoderKey = self.pointDecoderKey;
        end
        ...
        function setModelConfigs(self, configSet)
            ...
            self.pointDecoderModel = configSet.pointDecoderModel;
            self.pointDecoderKey = configSet.pointDecoderKey;
        end
