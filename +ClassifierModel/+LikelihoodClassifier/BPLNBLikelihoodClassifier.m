classdef BPLNBLikelihoodClassifier < handle
    ...
    methods
        function obj = BPLNBLikelihoodClassifier(sigmaA, sigmaB, stimCenter)
        ...
        function logLRatio = getLogLRatio(self, decodeOri, likelihood)
            [s_hat, ~] = self.cwExtractor(decodeOri, likelihood); % extract center and width of the likelihood function
            sigma = zeros(size(s_hat));
        ...
        function muLL = train(self, decodeOri, likelihood, classResp, nReps)
            fprintf('Training BPLNB-likelihood Classifier');
