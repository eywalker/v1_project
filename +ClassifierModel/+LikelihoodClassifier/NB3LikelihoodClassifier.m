classdef NB3LikelihoodClassifier < handle
    % FULLLIKELIHOODCLASSIFIER Fully Bayesian approach towards calculating
    % posterior over stimulus class given full likelihood L(s; r)
    properties
    ...
    methods
        function obj = NB3LikelihoodClassifier(sigmaA, sigmaB, stimCenter)
            ...
            obj.peakExtractor = @ClassifierModel.fitGaussToLikelihood;
        end
        ...
        function logLRatio = getLogLRatio(self, decodeOri, likelihood)
            ...
            logPrA = -1/2 * log(2*pi) - log(self.sigmaA) - (s_hat-self.stimCenter).^2 ./ 2 ./ (self.sigmaA^2);
            logPrB = -1/2 * log(2*pi) - log(self.sigmaB) - (s_hat-self.stimCenter).^2 ./ 2 ./ (self.sigmaB^2);
            logLRatio = logPrA - logPrB;
        end
        
        function muLL = train(self, decodeOri, likelihood, classResp, nReps)
            fprintf('Training NB3-likelihood Classifier');
