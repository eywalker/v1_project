classdef BPL2LikelihoodClassifier < handle
    ...
    methods
        ...
        function obj = BPL2LikelihoodClassifier(sigmaA, sigmaB, stimCenter)
        ...
        function logLRatio = getLogLRatioHelper(self, s_hat, sigma)
            logPrA = -1/2 * log(2*pi) - 1 / 2 * log(sigma.^2 + self.sigmaA^2) - s_hat.^2 ./ 2 ./ (sigma.^2 + self.sigmaA^2);
            logPrB = -1/2 * log(2*pi) - 1 / 2 * log(sigma.^2 + self.sigmaB^2) - s_hat.^2 ./ 2 ./ (sigma.^2 + self.sigmaB^2);
            logLRatio = logPrA - logPrB; % log(p(r | C = 'A') / p(r | C = 'B'))
        end
        ...
        function muLL = train(self, decodeOri, likelihood, classResp, nReps)
            fprintf('Training BPL2 Classifier');
