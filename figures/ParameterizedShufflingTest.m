...
plot(x, ones(size(x)) * log(0.5), 'k--');
title(sprintf('Performance on shuffled set vs contrast relative to %s', modelNames{target}));
legend(modelNames(models_to_plot));
xlabel('Contrast');
set(gca, 'xscale', 'log');
xlim([0.003, 1.2]);
...
plot(x, zeros(size(x)), 'k--');
title(sprintf('Performance on shuffled data vs contrast relative to %s', modelNames{target}));
legend(modelNames(models_to_plot));
xlabel('Contrast');
set(gca, 'xscale', 'log');
xlim([0.003, 1.2]);
