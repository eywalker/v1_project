...
title('Simulated Monkey T', 'FontName', font,'FontSize', fs_title);
...
set(ax, 'CLim', [-3, -0]);
set(ax, 'FontName', font, 'FontSize', fs);
title('Simulated Monkey L', 'FontName', font,'FontSize', fs_title);
xlabel('Non-Bayesian model: {\itP}(Correct)');
ylabel('Bayesian model: {\itP}(Correct)');
h = colorbar;
...
set(ax, 'Box', 'off');
xlabel('Contast (%)');
ylabel('Expected difference in {\itP}(Correct)');
xlim([0.002, 1]);
ylim([-0.01, 0.25]);
title('Simulated Monkey T', 'FontName', font, 'FontSize', fs_title);
