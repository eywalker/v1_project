...
sbtrainsets=pro(cd_dataset.SimBehCVTrainSets, 'lc_trainset_owner -> src_trainset_owner', 'lc_trainset_hash -> src_trainset_hash', 'lc_id -> src_id', '*');
train_leaf =  pro(cd_lc.TrainedLC, 'lc_train_mu_logl') * pro(sbtrainsets, 'dataset_hash -> lc_trainset_hash');
...
test_leaf =  pro(cd_lc.LCModelFits, 'lc_test_mu_logl') *  pro(sbtestsets, 'dataset_hash -> lc_testset_hash');
%%
...
filter = ~ismember(pk, {'lc_id', 'decoder_id', 'src_id'});
pk = pk(filter);
...
[trainLL, v_jointid, v_lcid, v_srcid, v_decid] = dj.struct.tabulate(train_data, 'train_mu_logl', 'joint_id', 'lc_id', 'src_id', 'decoder_id');
[testLL, v_jointid_test, v_lcid_test, v_srcid_test, v_decid_test] = dj.struct.tabulate(test_data, 'test_mu_logl', 'joint_id', 'lc_id', 'src_id', 'decoder_id');
